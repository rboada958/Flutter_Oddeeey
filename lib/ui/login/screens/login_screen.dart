...
import 'package:flutter_social/ui/home/home_screen.dart';
...
import 'package:flutter_social/ui/login/screens/register_screen.dart';
...
class _LoginState extends State<Login> {
  ...
  void getDataGoogle(RequestLogin request) {
    ...
    loginBloc.postLoginGoogle(request);
  }
    ...
    return Scaffold(
      body: StreamBuilder<ResponseLogin>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            if (snapshot.data.message != loginSuccess) {
              print("Data mensaje: ${snapshot.data.message}");
              return buildErrorWidget(snapshot.data.message, context, Login());
            }
            preferences.setApiToken(snapshot.data.data.api_token);
            return Home();
          } else if (snapshot.hasError) {
            print("Data mensaje hasError: ${snapshot.data.message}");
            return buildErrorWidget(snapshot.data.message, context, Login());
          } else {
            return _loginView();
          }
  ...
  Widget _fadeInImage() {
    return FadeInImage.assetNetwork(
      ...
      image: preferences.getData().logo,
      height: 150.0,
      width: 150.0,
    );
  }
    ...
    return Container(
      decoration: BoxDecoration(
        image: DecorationImage(
          image: NetworkImage(preferences.getData().background),
          fit: BoxFit.cover,
        ),
      ),
